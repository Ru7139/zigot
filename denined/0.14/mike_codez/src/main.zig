const std = @import("std");
// const print = @import("std").debug.print;

pub fn main() !void {
    // const StdOut = std.io.getStdOut().writer();
    // var out_buffer = std.io.bufferedWriter(StdOut);
    // var out_writer = out_buffer.writer();

    // fn

    const start_time = std.time.nanoTimestamp();

    const a: u32 = 10;
    const b: u32 = 20;
    std.debug.print("a = {}, b = {}\n", .{ a, b });

    const c1: u8 = 65;
    const c2: u8 = 'A';
    std.debug.assert(c1 == c2);

    const d1: u8 = 66;
    const d2: u8 = 'B';
    std.debug.print("\nd1 = {c}, d2 = {}\n", .{ d1, d2 });

    const e = "Here is zig language";
    std.debug.print("\ne: type = {}\n", .{@TypeOf(e)});
    std.debug.print("e: length = {}\n", .{e.len});
    std.debug.print("e: content = {s}\n", .{e});
    std.debug.print("e: last character = {c}\n", .{e[e.len - 1]});

    const f1 = [_]u32{ 1, 2, 3, 4, 5 };
    std.debug.print("\nf1: type = {}\n", .{@TypeOf(f1)});
    std.debug.print("f1: len = {}\n", .{f1.len});
    std.debug.print("f1: content = {any}\n", .{f1});

    const g1 = [_][]const u8{ "Apple", "Banana", "Cherry" };
    std.debug.print("\ng1: type = {}\n", .{@TypeOf(g1)});
    std.debug.print("g1: len = {}\n", .{g1.len});
    std.debug.print("g1: content = {s}\n", .{g1});

    const vec1 = @Vector(6, u32){ 10, 13, 17, 22, 28, 35 };
    const vec2 = @Vector(6, u32){ 1, 3, 5, 7, 9, 11 };
    const vec3 = vec1 + vec2; // value will be added, same type as themselves
    std.debug.print("\nvec3: type = {}\n", .{@TypeOf(vec3)});
    std.debug.print("vec3: len = {}\n", .{@typeInfo(@TypeOf(vec3)).vector.len});
    std.debug.print("vec3: content = {any}\n", .{vec3});
    // if add @Vector(6, u32) with @Vector(5, u32), it will get an error
    //      const vec3 = vec1 + vec2;
    //                   ~~~~~^~~~~~ (error: vector length mismatch)
    //      const vec3 = vec1 + vec2;
    //                   ^~~~ (length 6 here)
    //      const vec3 = vec1 + vec2;
    //                          ^~~~ length 5 here

    var value1: u32 = 10;
    const point1: *u32 = &value1;
    point1.* = 50;
    std.debug.print("\npoint1 = {d}\nvalue1 = {d}\n", .{ point1, value1 });

    const array1 = [_]u32{ 2, 3, 4, 5, 6 };
    const array2 = array1[1..4];
    std.debug.print("\n{}\n", .{@TypeOf(array1)});
    std.debug.print("{d}\n", .{array1});
    std.debug.print("{}\n", .{@TypeOf(array2)});
    std.debug.print("{d}\n", .{array2});

    const Rocket = struct {
        destination: []const u8,
        code: u8,
        index: u32,
    };
    const rocket_solar = Rocket{ .destination = "Atlantic", .code = 'U', .index = 9052 };
    std.debug.print("\nrocket_solar: type = {}\n", .{@TypeOf(rocket_solar)});
    std.debug.print("rocket_solar.destination = {s}\n", .{rocket_solar.destination});
    std.debug.print("rocket_solar.code = {c}\n", .{rocket_solar.code});
    std.debug.print("rocket_solar.index = {d}\n", .{rocket_solar.index});

    const ship_rock = .{ .ship_type = "FPSO", .destination = "New York", .code = 'P', .index = 2105 };
    std.debug.print("\nship_rock: type = {}\n", .{@TypeOf(ship_rock)});
    std.debug.print("ship_rock.type = {s}\n", .{ship_rock.ship_type});
    std.debug.print("ship_rock.destination = {s}\n", .{ship_rock.destination});
    std.debug.print("ship_rock.code = {c}\n", .{ship_rock.code});
    std.debug.print("ship_rock.index = {d}\n", .{ship_rock.index});

    const student_tommy_tuple = .{ "Tommy", "Sophomor", 20 };
    std.debug.print("\nstudent_tommy_tuple: type = {}\n", .{@TypeOf(student_tommy_tuple)});
    std.debug.print("\nstudent_tommy_tuple: content = {}\n", .{student_tommy_tuple});

    const Color = enum { Red, Green, Blue };
    const color1 = Color.Red;
    const color2 = Color.Blue;
    std.debug.print("\nthere are two kind of color {s} and {s}\n", .{ @tagName(color1), @tagName(color2) });

    const Num = union {
        int: u32,
        float: f32,
    };
    const num1 = Num{ .int = 10 };
    const num2 = Num{ .float = 2.712 };
    std.debug.print("\nnum1 = {d}, num2 = {d}\n", .{ num1.int, num2.float });

    const num3 = 10;
    switch (num3) {
        10 => std.debug.print("\n10 is not the correct value\n", .{}),
        12 => std.debug.print("\nthis is the correct value, {d}\n", .{num3}),
        else => std.debug.print("\nthis is not the correct value\n", .{}),
    }

    const Spot = enum { NewYork, Paris, Mosco, Tonga };
    const spot1 = Spot.Tonga;
    switch (spot1) {
        .NewYork => std.debug.print("\nWe create {s}\n", .{@tagName(spot1)}),
        .Paris => std.debug.print("\nWe have been {s}\n", .{@tagName(spot1)}),
        .Mosco => std.debug.print("\nWe fly to {s}\n", .{@tagName(spot1)}),
        .Tonga => std.debug.print("\nWe are at {s}\n", .{@tagName(spot1)}),
    }

    const MyUnion = union(enum) { number: u32, valid: bool };
    const my_union_1 = MyUnion{ .number = 100 };
    switch (my_union_1) {
        .number => |number| std.debug.print("\nGot {d}\n", .{number}),
        .valid => |v| std.debug.print("\nThe vaild = {}\n", .{v}),
    }

    const sign1 = 50;
    const sign2: ?u32 = switch (sign1) {
        10 => 20,
        20 => 100,
        else => null,
    };
    if (sign2) |value| {
        std.debug.print("\nThe value is {}\n", .{value});
    } else {
        std.debug.print("\nCan not find value in sign2\n", .{});
    }

    const final_elapsed_time_ns: i128 = std.time.nanoTimestamp() - start_time; // i128除法需要配置
    std.debug.print("\nExec Done with --> {}ns\n", .{final_elapsed_time_ns});
    // try out_buffer.flush();
}
